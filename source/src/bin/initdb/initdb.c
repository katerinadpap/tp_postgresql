...
static char *lineage_file; /* TPDB - KATERINA: File containing functions & aggregates*/
...
static void create_lineage(void); /* TPDB - KATERINA */
...
/* TPDB - KATERINA: The file lineage.sql is parsed and the functions and aggregates are included into postgres
 */
static void
create_lineage(void)
{
	...
	fputs(_("create lineage aggregate ... "), stdout);
		fflush(stdout);

	lines = readfile(lineage_file);
...
setup_data_file_paths(void)
{
	...
	set_input(&lineage_file, "lineage.sql"); /* TPDB - KATERINA: Set the name of the file */
	...
	check_input(lineage_file); 	/* TPDB - KATERINA: Check if the lineage.sql file exists */
