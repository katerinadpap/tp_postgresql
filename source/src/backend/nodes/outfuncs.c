...
/* TPDB - KATERINA */
static void
_outTimeAbsorption(StringInfo str, const Unique *node)
{
  int     i;

  WRITE_NODE_TYPE("TIMEABSORPTION");
...
/* TPDB - KATERINA */
static void
_outNegatingWindows(StringInfo str, const Alignment *node)
{
  int     i;

  WRITE_NODE_TYPE("NEGATING_WINDOWS");
  ...
  for (i = 0; i < node->numCols; i++)
    appendStringInfo(str, " %d", node->ColIdx[i]);

  appendStringInfo(str, " :eqOperators");
  for (i = 0; i < node->numCols; i++)
    appendStringInfo(str, " %u", node->eqOperators[i]);

  appendStringInfo(str, " :ltOperators");
    for (i = 0; i < node->numCols; i++)
      appendStringInfo(str, " %u", node->ltOperators[i]);
}


/* TPDB - KATERINA */
static void
_outAlignment(StringInfo str, const Alignment *node)
{
  int     i;

  WRITE_NODE_TYPE("ALIGNMENT");
...
/* TPDB - KATERINA */
static void
_outNormalization(StringInfo str, const Normalization *node)
{
  int     i;

  WRITE_NODE_TYPE("NORMALIZATION");
...
/* TPDB - KATERINA */
static void
_outTimeAdjustmentExpr(StringInfo str, const TimeAdjustmentExpr *node)
{
	WRITE_NODE_TYPE("TIMEADJUSTMENTEXPR");

	WRITE_INT_FIELD(timeAdjustmentType);
...
_outSelectStmt(StringInfo str, const SelectStmt *node)
{
	...
	WRITE_NODE_FIELD(timeAdjustmentOperation); /* TPDB - KATERINA */
	WRITE_NODE_FIELD(timeAbsorptionClause); /* TPDB - KATERINA */
	WRITE_ENUM_FIELD(lineage_type, LineageType); /* TPDB - KATERINA */
	WRITE_ENUM_FIELD(dimension_type, DimensionType); /* TPDB - KATERINA */
	WRITE_BOOL_FIELD(isTopLevelStmt); /* TPDB - KATERINA */
	...
	else
	{
		...
		switch (nodeTag(obj))
		{
			...
			case T_XmlSerialize:
				...
				break;
			case T_TimeAbsorption: /* TODB - KATERINA */
				_outTimeAbsorption(str, obj);
				break;
			case T_NegatingWindows: /* TODB - KATERINA */
				_outNegatingWindows(str, obj);
				break;
			case T_Alignment: /* TODB - KATERINA */
				_outAlignment(str, obj);
				break;
			case T_Normalization: /* TODB - KATERINA */
				_outNormalization(str, obj);
				break;
			case T_TimeAdjustmentExpr: /* TODB - KATERINA */
				_outTimeAdjustmentExpr(str, obj);
				break;
