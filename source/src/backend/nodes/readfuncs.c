...
_readQuery(void)
{
	...
	/* TPDB - KATERINA */
	READ_NODE_FIELD(timeAdjustmentOperation);
	READ_NODE_FIELD(timeAbsorptionClause);
...
_readTimeAdjustmentExpr(void)
{
  READ_LOCALS(TimeAdjustmentExpr);

  READ_INT_FIELD(timeAdjustmentType);
  READ_NODE_FIELD(usingClause);
  READ_NODE_FIELD(quals);
  READ_INT_FIELD(numOfRightCols); /* TPDB - KATERINA (needed for nullifying the right attributes) */
  READ_DONE();
}
...
#define MATCH(tokname, namelen) \
	...
	else if (MATCH("TIMEADJUSTMENTEXPR", 9)) /* TPDB - KATERINA */
	    return_value = _readTimeAdjustmentExpr();  /* TPDB - KATERINA */
