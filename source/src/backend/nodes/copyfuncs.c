...
/* TPDB - KATERINA */
static TimeAdjustmentExpr *
_copyTimeAdjustmentExpr(const TimeAdjustmentExpr *from)
{
	TimeAdjustmentExpr   *newnode = makeNode(TimeAdjustmentExpr);

	COPY_SCALAR_FIELD(timeAdjustmentType);
	COPY_NODE_FIELD(usingClause);
	COPY_NODE_FIELD(quals);
	COPY_SCALAR_FIELD(numOfRightCols); /* TPDB - KATERINA (needed for nullifying the right attributes) */
...
_copyQuery(const Query *from)
{
	...
	COPY_NODE_FIELD(timeAdjustmentOperation); /* TPDB - KATERINA */
	COPY_NODE_FIELD(timeAbsorptionClause); /* TPDB - KATERINA */
...
_copySelectStmt(const SelectStmt *from)
{
	...
	COPY_NODE_FIELD(timeAdjustmentOperation); 	/* TPDB - KATERINA */
	COPY_NODE_FIELD(timeAbsorptionClause);	/* TPDB - KATERINA */
	COPY_SCALAR_FIELD(lineage_type); 	/* TPDB - KATERINA */
	COPY_SCALAR_FIELD(dimension_type); 	/* TPDB - KATERINA */
//	COPY_SCALAR_FIELD(distinctLineageAsNotAnd);
	COPY_SCALAR_FIELD(isTopLevelStmt); /* TPDB - KATERINA */
//	COPY_SCALAR_FIELD(secondIteration);
...
copyObject(const void *from)
{
	...
	switch (nodeTag(from))
	{
		...
		case T_XmlSerialize:
			...
			break;
		case T_TimeAdjustmentExpr: /* TPDB - KATERINA */
			retval = _copyTimeAdjustmentExpr(from);
			break;
