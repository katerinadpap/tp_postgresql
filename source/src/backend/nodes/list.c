...
list_nth_cell(const List *list, int n) {
	...
	Assert(list != NIL);Assert(n >= 0);Assert(n < list->length);check_list_invariants(list);
...
// 1 for t1>t2, -1 for t1<t2, 0 for t1=t2
extern int cmp_tmsp(Datum t1, Datum t2) {
	return (DatumGetInt32(t1) > DatumGetInt32(t2) ) - (DatumGetInt32(t1) < DatumGetInt32(t2) );
}

/* TPDB - KATERINA
 * Returns (as previous) the last node whose te is less that te_check
 */
ListCell*
get_previous(const List *list, const void *te_check) {
	const ListCell *cell, *prev;

	Assert(IsPointerList(list)); check_list_invariants(list);

	//printf("Searching for prev\n");

	// Empty List
	if (list == NIL ) {
		//printf("A - First node to be added\n");
		return NULL ;
	}
	...
	foreach(cell, list)
	{
		dCoup = (datumCouple*) lfirst(cell);
		//printf("Comparing: %d -- %d -- %d\n", DatumGetInt32(dCoup->objB), DatumGetInt32(te_check), cmp_tmsp(dCoup->objB, te_check));

		// if cmp_tmsp(dCoup->objA, te_check) > 0 then dCoup->objA > te_check
		if (cmp_tmsp(dCoup->objB, te_check)>0) {
			//if (prev==NULL) printf("B - Add in the beginning\n");
			return prev;
		} else
			prev = cell;
	}

	//printf("C - Append in the End\n");
	return prev;
}


/* TPDB - KATERINA
 * deletes all nodes whose te is less that te_check
 */
List*
delete_all_less_than(const List *list, const void *te_check) {
	...
	do{
		dCoup = (datumCouple*) lfirst(list_head(list));
		// Comparing tes - Currently the struct has (te,lambda)
		//printf("Comparing: [%d,%d,%d]\n", DatumGetInt32(dCoup->objB), DatumGetInt32(te_check), cmp_tmsp(dCoup->objB, te_check));
		if (cmp_tmsp(dCoup->objB, te_check)<=0) {
			//printf("DELETE_IT\n");
...
bool list_member_int(const List *list, int datum) {
	...
	Assert(IsIntegerList(list));check_list_invariants(list);
...
bool list_member_oid(const List *list, Oid datum) {
	...
	Assert(IsOidList(list));check_list_invariants(list);
...
list_delete_cell(List *list, ListCell *cell, ListCell *prev) {
	check_list_invariants(list);Assert(prev != NULL ? lnext(prev) == cell : list_head(list) == cell);
...
list_union(const List *list1, const List *list2) {
	...
	Assert(IsPointerList(list1));Assert(IsPointerList(list2));
...
list_union_int(const List *list1, const List *list2) {
	...
	Assert(IsIntegerList(list1));Assert(IsIntegerList(list2));
...
list_union_oid(const List *list1, const List *list2) {
	...
	Assert(IsOidList(list1));Assert(IsOidList(list2));
