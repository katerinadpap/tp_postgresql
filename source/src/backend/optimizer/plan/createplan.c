...
/* TPDB - KATERINA */
Plan *
make_timeAdjustment(Plan *lefttree, TimeAdjustmentExpr *timeAdjustment)
{
	/*
	 * -> Updates execution cost and verifies that all OIDs of USING-Clause (If any) are valid. Add the equality/inequality operators including the column indexes and number of columns to the returned node <-
	 * Adjust the execution cost regarding the impact of the timeAdjustmentExpr
	 * The number of rows increase by a factor of 2 for normalization and 3 for alignment (Only difference between Normalization and Alignment)
	 * Therefore, the costs increase by 2 resp. 3 * numberOfCols
	 * Assert that all OIDs (object identifiers -> this are primary keys) are valid and add them to the Plan node
	 */
  if(timeAdjustment->timeAdjustmentType == 1 || timeAdjustment->timeAdjustmentType == 5) // Normalization or Set Operations
  {
	  Normalization   *node = makeNode(Normalization);
      Plan        *plan = &node->plan;
      int         numCols = timeAdjustment->usingClause->length;
      int         keyno = 0;
      AttrNumber  *ColIdx;
      Oid         *eqOperators,
                  *ltOperators;
      ...
      copy_plan_costsize(plan, lefttree); // startup_cost, total_cost, plan_rows (number of rows) and plan_width (average row width) get copied from lefttree to plan
      plan->plan_rows = plan->plan_rows * 2;
      ...
      Assert(numCols > 0);
      ColIdx = (AttrNumber *) palloc(sizeof(AttrNumber) * numCols);
      eqOperators = (Oid *) palloc(sizeof(Oid) * numCols);
      ltOperators = (Oid *) palloc(sizeof(Oid) * numCols);

      foreach(slitem, timeAdjustment->usingClause)
      {
        SortGroupClause *sortcl = (SortGroupClause *) lfirst(slitem);
        TargetEntry *tle = get_sortgroupclause_tle(sortcl, plan->targetlist);

        ColIdx[keyno] = tle->resno;
        eqOperators[keyno] = sortcl->eqop;
        Assert(OidIsValid(eqOperators[keyno]));
        ltOperators[keyno] = sortcl->sortop;
        Assert(OidIsValid(eqOperators[keyno]));
        keyno++;
      }
      ...
      node->numCols = numCols;
      node->ColIdx = ColIdx;
      node->eqOperators = eqOperators;
      node->ltOperators = ltOperators;

      return (Plan*) node;
  }

  else if(timeAdjustment->timeAdjustmentType == 2) // Alignment
  {
	  Alignment      *node = makeNode(Alignment);
      Plan        *plan = &node->plan;
      int         numCols = timeAdjustment->usingClause->length;
      int         numOfRightCols = timeAdjustment->numOfRightCols; /* TPDB - KATERINA (needed for nullifying the right attributes) */
      int         keyno = 0;
      AttrNumber  *ColIdx;
      Oid         *eqOperators,
                  *ltOperators;
      ...
      copy_plan_costsize(plan, lefttree); // startup_cost, total_cost, plan_rows (number of rows) and plan_width (average row width) get copied from lefttree to plan
      plan->plan_rows = plan->plan_rows * 3;
      ...
      //printf("OPTIMISING (%d) --- %d \n", timeAdjustment->timeAdjustmentType, list_length(plan->targetlist)); // TPDB - KATERINA
      ...
      node->eqOperators = eqOperators;
      node->ltOperators = ltOperators;
      node->numOfRightCols = numOfRightCols;  /* TPDB - KATERINA (needed for nullifying the right attributes) */

      return (Plan*) node;
  }
  else if(timeAdjustment->timeAdjustmentType == 6) // Unmatched/Overlapping/Negating Windows
  {
	  NegatingWindows      *node = makeNode(NegatingWindows);
      Plan        *plan = &node->plan;
      int         numCols = timeAdjustment->usingClause->length;
      int         numOfRightCols = timeAdjustment->numOfRightCols; /* TPDB - KATERINA (needed for nullifying the right attributes) */
      int         keyno = 0;
      ...
      node->eqOperators = eqOperators;
      node->ltOperators = ltOperators;
      node->numOfRightCols = numOfRightCols; // TPDB needed to make the right attributes null
