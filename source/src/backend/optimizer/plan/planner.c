...
grouping_planner(PlannerInfo *root, double tuple_fraction)
{
	...
	/* TPDB - KATERINA */
	/*
	 * Eliminate duplicates by sorting the plan first, before applying the make unique-method on the (required) sorted plan
	 * Adjust the number of result tuples (numberOfDistinctRows) for further query optimization
	 */
	if (parse->timeAbsorptionClause) {
		...
		result_plan = (Plan *) make_sort_from_pathkeys(root,
		                                  ...
		                                  current_pathkeys,
		                                     -1.0);
		// We use unique as a hull, since we perform the "same" operation
		dNumDistinctRows = result_plan->plan_rows; // Use statistics for it
		result_plan = (Plan *) make_unique(result_plan,
		                             parse->timeAbsorptionClause);
		result_plan->type = T_TimeAbsorption;
	...
	/* TPDB - KATERINA
	* If this is a timeAdjustment statement, adjust execution (plan) costs, verify OIDs and add all of them to a plan node
	*/
	if(parse->timeAdjustmentOperation)
	{
//
		//printf("IT REACHED THIS POINT\n");

	    TimeAdjustmentExpr * timeAdjustmentExpr = (TimeAdjustmentExpr *) parse->timeAdjustmentOperation;

		//printf("TIME_AD\n");
//
		result_plan = (Plan *) make_timeAdjustment(result_plan,
			   timeAdjustmentExpr);

		//printf("PLANNER MADE THE PLAN --- %d\n\n", list_length(result_plan->targetlist)); // TPDB

		//printf("RESULT PLAN READY\n");
//
//
	   current_pathkeys = make_pathkeys_for_sortclauses(root,
			   timeAdjustmentExpr->usingClause, tlist);
	}
...
standard_qp_callback(PlannerInfo *root, void *extra)
{
	...
	/*
	 * TPDB - KATERINA
	 * Generate a pathkeys list that represents the sort order specified by a list of SortGroupClauses
	 * We need this such that we can eliminate unnecessary duplicates when doing an absorption.
	 * (Also for updating the estimated execution costs)
	 */
	if(parse->timeAbsorptionClause)
		root->distinct_pathkeys =
				make_pathkeys_for_sortclauses(root,
