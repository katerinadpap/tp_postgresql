...
bool execTuplesMatch(TupleTableSlot *slot1, TupleTableSlot *slot2, int numCols, AttrNumber *matchColIdx,
	...
	for (i = numCols; --i >= 0;) {
		...
		printf("COMPARING: %ld -- %ld\nCOMPARING: %f -- %f\n", 
			 DatumGetInt64(attr1), DatumGetInt64(attr2), 
			 DatumGetFloat8(attr1), DatumGetFloat8(attr2));
		// printf("COMPARING: %s -- %s\n", 
		// 	DirectFunctionCall1(textout, DatumGetCString(attr1)),
		// 	DirectFunctionCall1(textout, DatumGetCString(attr2)));
		printf("EQUALITY = %d\n", DatumGetBool(FunctionCall2(&eqfunctions[i],
					attr1, attr2))  );
...
// TPDB - KATERINA
/*
 * execTuplesMatchAtPos
 *		Returns true if two tuples are definitely equal in the indicated
 *		field.
 *
 * Nulls are neither equal nor unequal to anything else.  A true result
 * is obtained only if there are non-null fields that compare not-equal.
 *
 * Parameters are identical to execTuplesMatch except for att that corresponds
 * to the position of the field to be compared.
 */
bool execTuplesMatchAtPos(TupleTableSlot *slot1, TupleTableSlot *slot2, int att, FmgrInfo *eqfunctions,
		MemoryContext evalContext) {
	MemoryContext oldContext;
	bool result;
	int i;
	...
	result = true;
	 ...
	 // AttrNumber att = matchColIdx[pos];
	Datum attr1, attr2;
	bool isNull1, isNull2;
	...
	printf("Are they null? %d ---- %d\n", isNull1,  isNull2);
	if (isNull1 != isNull2) return false; /* one null and one not; they aren't equal */
	printf("Checking Match: %s ---- %s\n", DatumGetCString(attr1),  DatumGetCString(attr2));
	...
	printf("EQUALITY = %d\n", DatumGetBool(FunctionCall2(&eqfunctions[i],
					attr1, attr2))  );
	if (!DatumGetBool(FunctionCall2(&eqfunctions[i],
					attr1, attr2)) ) 	result = false; /* they aren't equal */
...
// TPDB - KATERINA
/*
 * execTuplesMatchFacts
 *		Returns true if two tuples are definitely equal in the fields
 * 		corresponding to their fact, i.e. non-temporal, non-probabilistic
 *		attributes.
 *
 * Compared are the facts of tuples upcoming and cur_group. node is used
 * because it contains the positions when the fact attributes start and end.
 * Neg is used to determine if the tuples compared correspond to the join
 * result or to windows.
 */
bool execTuplesMatchFacts(TimeAdjustmentState *node, Alignment *plannode, TupleTableSlot *upcoming,
	TupleTableSlot *cur_group, int neg) {

	int counter = 0;
	bool sameGroup = false;

	for (counter = 0; counter < (node->rightFactEndPos-node->numOfRightCols); counter++) {
		Datum attr1, attr2; 
		bool isNull1, isNull2;
	
	
		AttrNumber att = plannode->ColIdx[counter];	
		attr1 = slot_getattr(cur_group, att, &isNull1);
		attr2 = slot_getattr(upcoming , att, &isNull2);
		
		D2 printf("NEG = %d --- Are they null BEFORE? %d ---- %d\n", neg, isNull1,  isNull2);
		if (isNull1 != isNull2) {
			sameGroup = false; 
			break;
		} /* one null and one not; they aren't equal */
	
		D2 printf("NEG = %d --- COMPARING: %ld -- %ld\n -- %f -- %f", neg,
			 DatumGetInt64(attr1), DatumGetInt64(attr2), 
			 DatumGetFloat8(attr1), DatumGetFloat8(attr2));

		if (!DatumGetBool(FunctionCall2(&node->eqfunctions[counter],
			attr1, attr2)) ){
			D2 printf(" -- NOT EQUAL\n");
			sameGroup = false; 
			return sameGroup;
	...
	if (neg == 0) {
		...
		AttrNumber att = plannode->ColIdx[node->tspos-1];
 ...
 */
bool execTuplesUnequal(TupleTableSlot *slot1, TupleTableSlot *slot2, int numCols, AttrNumber *matchColIdx, FmgrInfo *eqfunctions,
		MemoryContext evalContext) {
	MemoryContext oldContext;
	bool result;
	int i;
 ...
 */
FmgrInfo *
execTuplesMatchPrepare(int numCols, Oid *eqOperators) {
 ...
 */
TupleHashTable BuildTupleHashTable(int numCols, AttrNumber *keyColIdx, FmgrInfo *eqfunctions, FmgrInfo *hashfunctions, long nbuckets,
		Size entrysize, MemoryContext tablecxt, MemoryContext tempcxt) {
	TupleHashTable hashtable;
	HASHCTL hash_ctl;

	Assert(nbuckets > 0);Assert(entrysize >= sizeof(TupleHashEntryData));
	...
	hashtable->hashtab = hash_create("TupleHashTable", nbuckets, &hash_ctl, HASH_ELEM | HASH_FUNCTION | HASH_COMPARE | HASH_CONTEXT);
...
static int TupleHashTableMatch(const void *key1, const void *key2, Size keysize) {
	...
	if (execTuplesMatch(slot2, slot1, hashtable->numCols, hashtable->keyColIdx, hashtable->cur_eq_funcs, hashtable->tempcxt))
		return 0;
	else
		return 1;
}
