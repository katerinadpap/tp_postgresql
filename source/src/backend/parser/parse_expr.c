...
transformExprRecurse(ParseState *pstate, Node *expr)
{
	...
	check_stack_depth();
	// printf("Recursively transform = %d\n",nodeTag(expr) );
	switch (nodeTag(expr))
	{
		case T_ColumnRef:
		...
		case T_FuncCall:
			// printf("Transform the Function - ALMOST\n");
...
transformFuncCall(ParseState *pstate, FuncCall *fn)
{
	...
	ColumnRef *test = makeNode(ColumnRef);
	targs = NIL;
	foreach(args, fn->args)
	{

		if (IsA(lfirst(args), ColumnRef)) {
			test = (ColumnRef *) lfirst(args);
			if (test != NULL) {
				DB printf("name3 = %s\n", strVal((Value *) (linitial(test->fields))));			
			}
		}
