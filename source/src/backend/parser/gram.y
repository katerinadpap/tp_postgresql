...
%union
{
	...
	LineageType			lineage_type; /* TPDB - KATERINA */
	DimensionType		dimension_type; /* TPDB - KATERINA */
}
...
%type <list>	stmtblock stmtmulti
				...
				OptWith opt_distinct opt_absorb opt_definition func_args func_args_list /* TPDB - KATERINA */
...
/* TPDB - KATERINA */
%type <lineage_type>	opt_lineage

/* TPDB - KATERINA */
%type <dimension_type>	opt_dimension
...
%token <keyword> ABORT_P ABSOLUTE_P ABSORB ACCESS ACTION ADD_P ADMIN AFTER /* TPDB - KATERINA */
	AGGREGATE ALIGN ALL ALSO ALTER ALWAYS ANALYSE ANALYZE AND ANY ARRAY AS ASC /* TPDB - KATERINA */
	...
	COMMITTED CONCURRENTLY CONF CONFIGURATION CONNECTION CONSTRAINT CONSTRAINTS /* TPDB - KATERINA */
	...
	LEADING LEAKPROOF LEAST LEFT LEVEL LIKE LIMIT LINEAGE LISTEN LOAD LOCAL /* TPDB - KATERINA */
	LOCALTIME LOCALTIMESTAMP LOCATION LOCK_P

	MAPPING MATCH MATERIALIZED MAXVALUE MINUTE_P MINVALUE MODE MONTH_P MOVE

	NAME_P NAMES NATIONAL NATURAL NCHAR NEXT NO NONE NORMALIZE /* TPDB - KATERINA */
	...
	TO TP TRAILING TRANSACTION TREAT TRIGGER TRIM TRUE_P /* TPDB - KATERINA */
 ...
 */
simple_select:
			SELECT opt_distinct opt_absorb opt_lineage opt_dimension opt_target_list /* TPDB - KATERINA */
			...
			group_clause having_clause window_clause
				{
					...
					n->timeAbsorptionClause = $3; /* TPDB - KATERINA */
					n->lineage_type = $4; /* TPDB - KATERINA */
					n->dimension_type = $5; /* TPDB - KATERINA */
					printf("Lineage_Type = %d\n",n->lineage_type);
					printf("Dimension_Type = %d\n",n->dimension_type);
...
/* TPDB - KATERINA */
opt_absorb:
			ABSORB						{ $$ = list_make1(NIL); }
			| /*EMPTY*/					{ $$ = NIL; }
		;
			
/* TPDB - KATERINA */
opt_lineage:
			LINEAGE CONF					{ $$ = LINEAGE_BOTH; }
			| CONF LINEAGE					{ $$ = LINEAGE_BOTH; }
			| LINEAGE					{ $$ = LINEAGE_LINEAGE; }
			| CONF						{ $$ = LINEAGE_CONF; }
			| /*EMPTY*/					{ $$ = LINEAGE_NONE; }
		;
...
table_ref:	relation_expr opt_alias_clause
			...
			/* TPDB - KATERINA*/
			| '(' table_ref NORMALIZE table_ref USING '(' name_list ')' ')' alias_clause
        {
          RangeSubselect 		*n = makeNode(RangeSubselect);
          ColumnRef      		*cr = makeNode(ColumnRef);
          ResTarget      		*rt = makeNode(ResTarget);
          ...
          stmt->fromClause = list_make2($2, $4);
          
          timeAdjustment->timeAdjustmentType = 1; // Normalization 
          timeAdjustment->usingClause = $7;
          
          stmt->timeAdjustmentOperation = (Node *) timeAdjustment;
          
          n->subquery = (Node *) stmt;
          n->alias = $10;
          $$ = (Node *)n;
        }	
      | '(' table_ref NORMALIZE table_ref USING '(' ')' ')' alias_clause
        {
          RangeSubselect 		*n = makeNode(RangeSubselect);
          ColumnRef      		*cr = makeNode(ColumnRef);
          ResTarget      		*rt = makeNode(ResTarget);
      ...
      | '(' table_ref NORMALIZE table_ref ON a_expr ')' alias_clause
        {
          RangeSubselect 		*n = makeNode(RangeSubselect);
          ColumnRef      		*cr = makeNode(ColumnRef);
          ResTarget      		*rt = makeNode(ResTarget);
          ...
          timeAdjustment->timeAdjustmentType = 3; // Normalization on theta 
          timeAdjustment->quals = $6;
          timeAdjustment->usingClause = NIL;
      ...
      | '(' table_ref ALIGN table_ref ON a_expr ')' alias_clause
        {
          RangeSubselect 		*n = makeNode(RangeSubselect);
          ColumnRef      		*cr = makeNode(ColumnRef);
          ResTarget      		*rt = makeNode(ResTarget);
          ...
          timeAdjustment->timeAdjustmentType = 2; // Alignment 
          timeAdjustment->usingClause = NIL;
          timeAdjustment->quals = $6;
...
reserved_keyword:
			...
			| LEADING
			| LIMIT
			| LINEAGE /* TPDB - KATERINA */
			| LOCALTIME
			| LOCALTIMESTAMP
			| NORMALIZE /* TPDB - KATERINA */
			| NOT
			| NULL_P
			| OFFSET
			| ON
...
makeSetOp(SetOperation op, bool all, Node *larg, Node *rarg)
{
	...
	/* TPDB - KATERINA */
	n->lineage_type = n->larg->lineage_type;

	if (n->larg->dimension_type != DIMENSION_NONE || n->rarg->dimension_type != DIMENSION_NONE) {
		n->dimension_type = DIMENSION_TP;
		n->larg->dimension_type = DIMENSION_NONE;
		n->rarg->dimension_type = DIMENSION_NONE;

		TimeAdjustmentExpr    *timeAdjustment = makeNode(TimeAdjustmentExpr);
        timeAdjustment->timeAdjustmentType = 4; // Adjustment for Set Operations 
        n->timeAdjustmentOperation = (Node *) timeAdjustment;

	}
	// n->larg->lineage_type = LINEAGE_NONE;
	// n->rarg->lineage_type = LINEAGE_NONE;
