...
transformTargetEntry(ParseState *pstate,
  ...
  //transformTargetEntry(pstate, res->val, NULL, exprKind, res->name,	false);
{
	/* Transform the node if caller didn't do it already */
	if (expr == NULL) {
		printf("THE CALLER DIDN'T TRANSFORM --- %d\n", exprKind);
					...
					ParseExprKind exprKind)
{
	...
	printf("transform the target - Check 1\n");

	foreach(o_target, targetlist)
	{

		printf("transform the target - Check 2a\n");
		...
		printf("transform the target - Check 2b\n");


		//printf("sthELSE - res = %s\n", res->name);
	...
	printf("transform the target - Check 3\n");
