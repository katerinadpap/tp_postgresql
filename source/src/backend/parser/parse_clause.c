...
/* TPDB - KATERINA*/
extern void setNamespaceLateralState(List *namespace,
						 bool lateral_only, bool lateral_ok);
...
transformFromClause(ParseState *pstate, List *frmList)
{
	...
	DB printf("transform the from - Check 1\n");
	...
	DB printf("transform the from - Check 2\n");
	...
	DB printf("transform the from - Check 3\n");
...
/* TPDB - KATERINA (I made it extern)*/
extern Node *
						...
						List **namespace)
{
	...
	else if (IsA(n, RangeSubselect))
	{
		...
		DB printf("It's a Rangesubselect\n");
	...
	else if (IsA(n, RangeFunction))
	{
		...
		DB printf("It's a RangeFunction\n");
...
/* TPDB - KATERINA
 * Add targetList to usingClause
 * set resjunk of the last (alignment) respectively the two last (normalization) elements to true
 * 			-> to eliminate the attribute from final target list
 */
void
transformTimeAdjustmentClause(ParseState *pstate,
            List **targetlist, TimeAdjustmentExpr *timeAdjustmentExpr)
{
  ListCell  *tlitem;
  int       attnum = 1;

  timeAdjustmentExpr->usingClause = NIL;
  foreach(tlitem, *targetlist)
  {
    TargetEntry *tle = (TargetEntry *) lfirst(tlitem);

    timeAdjustmentExpr->usingClause = addTargetToGroupList(pstate, tle,
    						  timeAdjustmentExpr->usingClause, *targetlist,
                              exprLocation((Node *) tle->expr),
                              true);

    if(timeAdjustmentExpr->timeAdjustmentType == 1 || timeAdjustmentExpr->timeAdjustmentType == 5 ) // Normalization or Set Operations
    {
      if (attnum == (*targetlist)->length+1) /* TPDB - KATERINA */
        tle->resjunk = true;
    } else if (timeAdjustmentExpr->timeAdjustmentType == 2 || timeAdjustmentExpr->timeAdjustmentType == 6) // Alignment or Unmatched/Overlapping/Negating Windows
    {
        if(attnum >= (*targetlist)->length+1) /* TPDB - KATERINA */
          tle->resjunk = true;
    } else
    {
      ereport(ERROR,
         (errcode(ERRCODE_INTERNAL_ERROR),
          errmsg("undefined time adjustment type %d !",
        		  timeAdjustmentExpr->timeAdjustmentType)));
    }
    attnum++;
  }
}
