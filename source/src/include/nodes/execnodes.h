...
typedef enum
{
	OVERLAPPING_UNMATCHED_JOINS,
	NEGATING_JOINS,
	OVERLAPPING_UNMATCHED_SET,
	NEGATING_SET
} WindowType;


/* TPDB - KATERINA */
typedef struct TimeAdjustmentState
{
  ScanState 	ss;       		// its first field is NodeTag
  MemoryContext tempContext;  	// short-term context for comparisons

  bool 			firstCall;
  WindowType 	windowsToBeProduced; /* TPDB */
  bool			alignment;
  bool 			done;

  bool 			sameGroup;

  Datum 		sweep; // sweep line for the left tuple
  Datum 		breakPoint; // sweep line for the left tuple
  ...
  bool 			negating; // checks if a negating window should be returned
  List 			*PQ;
  Datum 		prevWindTe;

  datumCouple	*rTuple; // (ts, te, lineage) for all "adjustment_wise active" tuples in r
  datumCouple	*sTuple; // (ts, te, lineage) for all "adjustment_wise active" tuples in s
