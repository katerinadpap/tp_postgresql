...
/* TPDB - KATERINA */
typedef struct NegatingWindows
{
  Plan    		plan;
  int     		numCols;    	// number of columns in total
  AttrNumber 	*ColIdx;   		// their indexes in the target list
  Oid      		*eqOperators;  	// equality operators to compare with
  Oid      		*ltOperators;  	// equality operators to compare with
  int 			type; 			// 1 NORMALIZATION, 2 ALIGN to be corrected
  int 			numOfRightCols; /* TPDB - KATERINA (needed for nullifying the right attributes) */
} NegatingWindows;
...
/* TPDB - KATERINA */
typedef struct TimeIntervalAdjustment
{
  Plan    		plan;
  int     		numCols;    	// number of columns in total
  AttrNumber 	*ColIdx;   		// their indexes in the target list
